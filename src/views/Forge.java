...
public class Forge extends JFrame {
	...
	public Forge() {
		...
		JButton upgradeItem = new JButton("UPGRADE ITEM");
		...
		upgradeItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(actuallyBagItemList.get(numberOfChooseItem).gold *100 <= Character.getGold()){
				Character.costGold(actuallyBagItemList.get(numberOfChooseItem).gold *100); //cost of upgrade
				if(Character.bag.get(numberOfChooseItem).itemLVL<10)
				Item.reloadItemStats(Character.bag.get(numberOfChooseItem)); // cannot upgrade item upper than 10 lvl
				else JOptionPane.showMessageDialog(contentPane, "YOU HAVE REACHED MAX LVL OF THIS ITEM!");
				}
		...
		JTextArea upgradeCost = new JTextArea();
		upgradeCost.setEditable(false);
		upgradeCost.setBounds(454, 580, 331, 60);
		upgradeCost.setFont(new Font("Monotype Corsiva", Font.PLAIN, 30));
		contentPane.add(upgradeCost);
		...
		chooseItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				upgradeCost.setText(" Upgrade cost: " + actuallyBagItemList.get(numberOfChooseItem).gold *100 + " gold coins\n");
			}
		});
